$(document).ready(function(){
		...
		$.ajax({
		  ...
		  success: function(response){
		    ...
		    if (response == 1) {			    
			    ...
			    $('.alert_dev').html('<p class="alert alert-success">Un nuevo dispositivo fue agregado exitosamente</p>');
	...
	$(document).on('click', '.dev_uid_up', function(){
		...
		bootbox.confirm("Deseas de verdad actualizar este codigo de dispositivo?", function(result) {
			if(result){
			     // AJAX Request
			     $.ajax({
			       url: 'dev_config.php',
			       ...
			       success: function(response){
			        ...
			        if(response == 1){			    
				    	$('.alert_dev').fadeIn(500);
			        	$('.alert_dev').html('<p class="alert alert-success">Un nuevo codigo fue asigando exitosamente</p>');
	...
	$(document).on('click', '.dev_del', function(){
		...
		bootbox.confirm("Deseas de verdad eliminar este dispositivo?", function(result) {
		if(result){
		     // AJAX Request
		     $.ajax({
		       url: 'dev_config.php',
	...
	$(document).on('click', '.mode_sel', function(){
		...
		bootbox.confirm("Realmente quieres cambiar este modo de dispositivo?", function(result) {
			if(result){
			     // AJAX Request
			     $.ajax({
			       url: 'dev_config.php',
			       ...
			       success: function(response){
			          	...
			          	if(response == 1){
			          		// bootbox.alert('<p class="alert alert-success">El modo de dispositivo se ha actualizado correctamente</p>');
			              	$(el).closest('tr').css('background','#5cb85c');
					        $(el).closest('tr').fadeOut(300,function(){
					              $(this).show();
					        });
